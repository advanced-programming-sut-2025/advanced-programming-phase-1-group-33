    ...
    public void handleCreateLobby(String name, boolean isPrivate, String password, boolean isVisible) {
        new Thread(() -> {
            try {
                ...
                ResponseWrapper responseWrapper = (ResponseWrapper) responseHolder.getResponse(5000); // Wait for 5
                                                                                                      // seconds
                ...
                if (responseWrapper != null && responseWrapper.getType() == ResponseType.SUCCESSFUL) {
                    ...
                    String message = (responseWrapper != null) ? responseWrapper.getPayload()
    ...
    public void handleRefreshLobbies() {
        new Thread(() -> {
            try {
                ...
                RefreshLobbiesResponse response = (RefreshLobbiesResponse) connectionManager
                        .sendRequestAndWaitForResponse(RequestType.REFRESH_LOBBIES, null, 500); // Second call
                System.out.println(response);
                if (response != null ) {
                    // RefreshLobbiesResponse response = gson.fromJson(responseWrapper.getPayload(),
                    //         RefreshLobbiesResponse.class);
                    Gdx.app.postRunnable(() -> view.updateLobbyList(response.getVisibleLobbies()));
                } else {
                    // String message = (responseWrapper != null) ? responseWrapper.getPayload()
                    //         : "Connection timeout or unknown error.";
                    Gdx.app.postRunnable(() -> view.showMessage("Failed to refresh lobbies: " + response, 0, 0));
                }
            } catch (InterruptedException e) {
                Gdx.app.postRunnable(() -> view.showMessage("Request interrupted.", 0, 0));
            }
    ...
    public void handleSearchLobby(String lobbyId) {
        new Thread(() -> {
            try {
                ...
                if (responseWrapper != null && responseWrapper.getType() == ResponseType.SUCCESSFUL) {
                    SearchLobbyResponse response = gson.fromJson(responseWrapper.getPayload(),
                            SearchLobbyResponse.class);
                    String message = (response.getFoundLobby() != null)
                            ? "Lobby found: " + response.getFoundLobby().getName()
                            : response.getMessage();
