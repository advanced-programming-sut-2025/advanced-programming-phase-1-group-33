...
public class EdibleArtisanAnimation {
    private final TextureRegion texture;
    private final Vector2 startPosition;
    private final EdibleArtisanProduct Item;
    ...
    public EdibleArtisanAnimation(EdibleArtisanProduct Item, Player player, float duration) {
        this.texture = Item.getTextureRegion(GameAssetManager.getInstance());
    ...
    public boolean update(float delta) {
        ...
        if (!effectApplied && position.y <= player.playerPosition.y + 16) {
            ...
            player.getBackpack().getInventory().reduceItemQuantity(Item, 1);
            effectApplied = true;
        }
