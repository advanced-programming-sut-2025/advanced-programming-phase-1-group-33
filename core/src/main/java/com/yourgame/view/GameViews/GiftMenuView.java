...
public class GiftMenuView extends Window {
    ...
    public GiftMenuView(Skin skin, Stage stage, GameScreen gameScreen, NPC npc) {
        super("Gift Menu", skin);
        this.skin = skin;
        this.stage = stage;
        ...
        giftButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                else if(!(selectedItem.item() instanceof Food)){
                    gameScreen.showMessage("error","You must gift a food!",skin,0,200,stage);
                }
                else {
                    updateInventory(selectedItem,npc);
        ...
        mainTable.add(giftButton).left().padLeft(20);
    ...
    private void updateInventory(InventorySlot inventorySlot,NPC npc) {
        if(inventorySlot == null) return;

        Player player = gameScreen.getPlayer();

        switch (npc.getType()){
            case Harvey -> player.setRelationWithHarvey(player.getRelationWithHarvey() + 2);
            case Leah -> player.setRelationWithLeah(player.getRelationWithLeah() + 2);
            case Sebastian -> player.setRelationWithSebastian(player.getRelationWithSebastian() + 2);
            case Robin -> player.setRelationWithRobin(player.getRelationWithRobin() + 2);
            case Pierre -> player.setRelationWithPierre(player.getRelationWithPierre() + 2);
        }
